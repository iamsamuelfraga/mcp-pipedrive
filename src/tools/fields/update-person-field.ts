...
import { UpdatePersonFieldSchema } from '../../schemas/person-field.js';
import type { PipedriveResponse } from '../../types/common.js';

export function getUpdatePersonFieldTool(client: PipedriveClient) {
  return {
    fields_update_person_field: {
      description: `Update an existing custom person field.
...
- Toggle the field's visibility in the add form`,
      inputSchema: {
        ...
        properties: {
          id: {
            type: 'number',
            description: 'ID of the person field to update',
          },
          name: {
            type: 'string',
      ...
      handler: async (args: unknown) => {
        const parsed = UpdatePersonFieldSchema.parse(args);
