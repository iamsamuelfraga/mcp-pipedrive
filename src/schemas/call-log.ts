...
export const CreateCallLogSchema = z
  .object({
    ...
    start_time: DateTimeStringSchema.describe('Call start time in ISO 8601 format'),
    end_time: DateTimeStringSchema.describe('Call end time in ISO 8601 format'),
    user_id: OptionalIdSchema.describe('ID of the user who made the call'),
    activity_id: OptionalIdSchema.describe('ID of the associated activity'),
    subject: z
      .string()
      .max(255, 'Subject cannot exceed 255 characters')
      .optional()
      .describe('Call subject/title'),
    duration: z.string().optional().describe('Call duration (e.g., "120" for 120 seconds)'),
    from_phone_number: z.string().optional().describe('Phone number that made the call'),
    person_id: OptionalIdSchema.describe('ID of the person associated with the call'),
    org_id: OptionalIdSchema.describe('ID of the organization associated with the call'),
    deal_id: OptionalIdSchema.describe('ID of the deal associated with the call'),
    lead_id: z
      .string()
...
export const GetCallLogSchema = z.object({
  id: z.string().min(1, 'Call log ID is required').describe('ID of the call log to retrieve'),
});
...
export const DeleteCallLogSchema = z.object({
  id: z.string().min(1, 'Call log ID is required').describe('ID of the call log to delete'),
});
