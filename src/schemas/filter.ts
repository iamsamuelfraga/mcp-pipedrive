...
export const FilterConditionLeafSchema = z.object({
  object: z.string().describe('Object type (e.g., deal, person, org)'),
  field_id: z.string().describe('Field ID or field key'),
...
export const FilterConditionGroupSchema: z.ZodType<{
  glue: 'and' | 'or';
  conditions: (FilterConditionLeaf | { glue: 'and' | 'or'; conditions: any[] } | null)[];
}> = z.lazy(() =>
  z.object({
    glue: z
      .enum(['and', 'or'], {
    ...
    conditions: z
      .array(z.union([FilterConditionLeafSchema, FilterConditionGroupSchema, z.null()]))
...
export const CreateFilterSchema = z
  .object({
    ...
    conditions: FilterConditionsSchema.describe('Filter conditions structure'),
    type: FilterTypeSchema.describe(
      'The type of filter (deals, org, people, products, activities)'
    ),
  })
  .strict();
...
export const UpdateFilterSchema = z
  .object({
    id: IdSchema.describe('ID of the filter to update'),
    name: z
      .string()
      ...
      .optional()
      .describe('Filter name'),
    conditions: FilterConditionsSchema.optional().describe('Filter conditions structure'),
  })
  .strict();
...
export const ListFiltersSchema = z
  .object({
    type: FilterTypeSchema.optional().describe(
      'Filter by type (deals, org, people, products, activities)'
    ),
  })
  .strict();
...
export const GetFilterSchema = z
  .object({
    id: IdSchema.describe('ID of the filter to retrieve'),
  })
  .strict();
...
export const DeleteFilterSchema = z
  .object({
    id: IdSchema.describe('ID of the filter to delete'),
  })
  .strict();
 ...
 */
export const BulkDeleteFiltersSchema = z
  .object({
    ids: z
      .string()
