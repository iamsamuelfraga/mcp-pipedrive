...
export const UploadFileSchema = z
  .object({
    ...
    deal_id: OptionalIdSchema.describe('ID of the deal to attach the file to'),
    person_id: OptionalIdSchema.describe('ID of the person to attach the file to'),
    org_id: OptionalIdSchema.describe('ID of the organization to attach the file to'),
    activity_id: OptionalIdSchema.describe('ID of the activity to attach the file to'),
    product_id: OptionalIdSchema.describe('ID of the product to attach the file to'),
  })
  .strict()
  .refine(
    (data) => {
    ...
    {
      message:
        'At least one of deal_id, person_id, org_id, activity_id, or product_id must be provided',
      path: ['deal_id'],
    }
  );
 ...
 */
export const UploadFileFromPathSchema = z
  .object({
    file_path: z.string().min(1, 'File path is required').describe('Path to the file to upload'),
    deal_id: OptionalIdSchema.describe('ID of the deal to attach the file to'),
    person_id: OptionalIdSchema.describe('ID of the person to attach the file to'),
    org_id: OptionalIdSchema.describe('ID of the organization to attach the file to'),
    activity_id: OptionalIdSchema.describe('ID of the activity to attach the file to'),
...
export const GetFileSchema = z
  .object({
    id: IdSchema.describe('ID of the file to retrieve'),
  })
  .strict();
...
export const DownloadFileSchema = z
  .object({
    id: IdSchema.describe('ID of the file to download'),
  })
  .strict();
...
export const DeleteFileSchema = z
  .object({
    id: IdSchema.describe('ID of the file to delete'),
  })
  .strict();
...
export const UpdateFileSchema = z
  .object({
    id: IdSchema.describe('ID of the file to update'),
    name: z
      .string()
...
export const ListFilesSchema = PaginationSchema.extend({
  ...
  activity_id: OptionalIdSchema.describe('Filter by activity ID'),
  product_id: OptionalIdSchema.describe('Filter by product ID'),
  include_deleted_files: z
    .boolean()
    .optional()
    .default(false)
    .describe('Whether to include deleted files'),
  sort: z.string().optional().describe('Field to sort by (e.g., add_time, update_time, file_name)'),
 ...
 */
export const LinkRemoteFileSchema = z
  .object({
    item_type: z
 ...
 */
export const UnlinkRemoteFileSchema = z
  .object({
