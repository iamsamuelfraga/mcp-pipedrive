...
export const ExpectedOutcomeSchema = z.object({
  ...
  currency_id: OptionalIdSchema.describe('Currency ID if tracking monetary value'),
});
...
export const GoalDurationSchema = z.object({
  start: DateStringSchema.describe('Start date in YYYY-MM-DD format'),
  end: DateStringSchema.describe('End date in YYYY-MM-DD format'),
});
...
export const CreateGoalSchema = z
  .object({
    title: z
      ...
      .describe('Goal title'),
    type: GoalTypeSchema.describe('Goal type configuration'),
    assignee: GoalAssigneeSchema.describe('User or team assigned to this goal'),
    expected_outcome: ExpectedOutcomeSchema.describe('Expected outcome configuration'),
...
export const UpdateGoalSchema = z
  .object({
    id: z.string().min(1, 'Goal ID is required').describe('ID of the goal to update'),
    title: z
      .string()
      ...
      .optional()
      .describe('Goal title'),
    type: GoalTypeSchema.optional().describe('Goal type configuration'),
    assignee: GoalAssigneeSchema.optional().describe('User or team assigned to this goal'),
    expected_outcome: ExpectedOutcomeSchema.optional().describe('Expected outcome configuration'),
...
export const GetGoalSchema = z.object({
  id: z.string().min(1, 'Goal ID is required').describe('ID of the goal to retrieve'),
});
...
export const DeleteGoalSchema = z.object({
  id: z.string().min(1, 'Goal ID is required').describe('ID of the goal to delete'),
});
...
export const ListGoalsSchema = z
  .object({
    'type.name': z.string().optional().describe('Filter by goal type name'),
...
export const GetGoalResultsSchema = z.object({
  id: z.string().min(1, 'Goal ID is required').describe('ID of the goal to get results for'),
  'period.start': DateStringSchema.describe('Period start date in YYYY-MM-DD format'),
  'period.end': DateStringSchema.describe('Period end date in YYYY-MM-DD format'),
});
