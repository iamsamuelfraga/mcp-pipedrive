...
export const CreateActivitySchema = z
  .object({
    ...
    type: ActivityTypeSchema.describe('Type of the activity'),
    due_date: DateStringSchema.describe('Due date of the activity in YYYY-MM-DD format'),
    due_time: z
      .string()
    ...
    user_id: OptionalIdSchema.describe('ID of the user this activity is assigned to'),
    deal_id: OptionalIdSchema.describe('ID of the deal this activity is associated with'),
    person_id: OptionalIdSchema.describe('ID of the person this activity is associated with'),
    org_id: OptionalIdSchema.describe('ID of the organization this activity is associated with'),
    location: z
      .string()
    ...
    done: BooleanLikeSchema.optional()
      .default(false)
    ...
    participants: z
      .array(
        z.object({
          ...
          primary_flag: BooleanLikeSchema.optional()
            .default(false)
    ...
    busy_flag: BooleanLikeSchema.optional().describe(
      'Whether the activity shows as busy in calendar'
    ),
    attendees: z
      .array(
        z.object({
  ...
  .refine(
    (data) => {
      ...
      return (
        data.deal_id !== undefined || data.person_id !== undefined || data.org_id !== undefined
      );
    },
    {
...
export const UpdateActivitySchema = z
  .object({
    id: IdSchema.describe('ID of the activity to update'),
    subject: z
      .string()
      ...
      .optional()
      .describe('Subject/title of the activity'),
    type: ActivityTypeSchema.optional().describe('Type of the activity'),
    due_date: DateStringSchema.optional().describe('Due date of the activity in YYYY-MM-DD format'),
    due_time: z
      .string()
      .regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'Due time must be in HH:MM format (24-hour)')
      .optional()
    ...
    public_description: z
      ...
      .optional()
      .describe('Public description visible to participants'),
    done: BooleanLikeSchema.optional().describe('Whether the activity is marked as done'),
    participants: z
      .array(
        z.object({
          person_id: IdSchema.describe('ID of the participant'),
...
export const ListActivitiesSchema = PaginationSchema.extend({
  user_id: OptionalIdSchema.describe('Filter by user (assignee) ID'),
  deal_id: OptionalIdSchema.describe('Filter by deal ID'),
  ...
  start_date: DateStringSchema.optional().describe('Start date for filtering (YYYY-MM-DD)'),
  end_date: DateStringSchema.optional().describe('End date for filtering (YYYY-MM-DD)'),
  filter_id: OptionalIdSchema.describe('ID of the filter to use'),
  sort: z.string().optional().describe('Field to sort by (e.g., due_date, add_time, subject)'),
...
export const GetActivitySchema = z
  .object({
    id: IdSchema.describe('ID of the activity to retrieve'),
  })
  .strict();
...
export const DeleteActivitySchema = z
  .object({
    id: IdSchema.describe('ID of the activity to delete'),
  })
  .strict();
 ...
 */
export const MarkActivityDoneSchema = z
  .object({
    id: IdSchema.describe('ID of the activity to mark as done'),
  })
  .strict();
 ...
 */
export const MarkActivityUndoneSchema = z
  .object({
    id: IdSchema.describe('ID of the activity to mark as undone'),
  })
  .strict();
 ...
 */
export const BulkDeleteActivitiesSchema = z
  .object({
    ids: z
      .array(IdSchema)
 ...
 */
export const BulkUpdateActivitiesSchema = z
  .object({
    ids: z
      .array(IdSchema)
      .min(1, 'At least one activity ID is required')
    ...
    user_id: OptionalIdSchema.describe('New assignee user ID'),
    done: BooleanLikeSchema.optional().describe('Mark activities as done/undone'),
    type: ActivityTypeSchema.optional().describe('New activity type'),
  })
  .strict();
 ...
 */
export const GetActivitiesCollectionSchema = z
  .object({
    ...
    since: DateTimeStringSchema.optional().describe(
      'Filter activities modified since this timestamp'
    ),
    until: DateTimeStringSchema.optional().describe(
      'Filter activities modified until this timestamp'
    ),
  })
  .strict();
