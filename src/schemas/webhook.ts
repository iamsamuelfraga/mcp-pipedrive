...
export const EventActionSchema = z.enum([
  ...
  errorMap: () => ({ message: 'Event action must be one of: added, updated, deleted, merged, or * (all)' }),
});
...
export const EventObjectSchema = z.enum([
  ...
  errorMap: () => ({ message: 'Event object must be one of: activity, activityType, deal, note, organization, person, pipeline, product, stage, user, or * (all)' }),
});
...
export const CreateWebhookSchema = z.object({
  ...
  event_action: EventActionSchema
    .describe('The type of action that triggers the webhook (added, updated, deleted, merged, or *)'),
  event_object: EventObjectSchema
    .describe('The type of object that triggers the webhook (activity, deal, person, organization, etc., or *)'),
  user_id: OptionalIdSchema
    .describe('The ID of the user this webhook will be associated with'),
  http_auth_user: z.string()
    .optional()
...
export const GetWebhookSchema = z.object({
  id: IdSchema
    .describe('ID of the webhook to retrieve'),
}).strict();
