 ...
 */
export const UniversalSearchSchema = z
  .object({
    term: z
      .string()
      .min(
        1,
        'Search term must be at least 1 character when exact_match is true, 2 characters otherwise'
      )
      .max(255, 'Search term cannot exceed 255 characters')
      .describe('Search term'),
    item_types: z
      .array(SearchItemTypeSchema)
      .optional()
      .describe('Array of item types to search (deal, person, organization, product, lead, file)'),
    fields: z.string().optional().describe('Comma-separated list of field names to search'),
    exact_match: z
      .boolean()
      .optional()
      .default(false)
      .describe('Whether to perform an exact match search'),
    search_for_related_items: z
      .boolean()
      .optional()
      .default(true)
 ...
 */
export const SearchByFieldSchema = z
  .object({
    term: z
      .string()
      .min(
...
export const SearchDealsSchema = z
  .object({
    ...
    exact_match: z
      ...
      .default(false)
      .describe('Whether to perform an exact match search'),
    person_id: OptionalIdSchema.describe('Filter by associated person ID'),
    org_id: OptionalIdSchema.describe('Filter by associated organization ID'),
    status: z
