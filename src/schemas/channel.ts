...
export const CreateChannelSchema = z
  .object({
    ...
    template_support: BooleanLikeSchema.optional()
      .default(false)
...
export const DeleteChannelSchema = z
  .object({
    id: z.string().min(1, 'Channel ID is required').describe('ID of the channel to delete'),
  })
  .strict();
...
export const MessageAttachmentSchema = z
  .object({
    id: z.string().min(1, 'Attachment ID is required').describe('ID of the attachment'),
    type: z
      .string()
    ...
    name: z.string().min(1, 'Attachment name is required').describe('Name of the attachment file'),
    size: z
      .number()
    ...
    link_expires: BooleanLikeSchema.optional()
      .default(false)
 ...
 */
export const ReceiveMessageSchema = z
  .object({
    id: z.string().min(1, 'Message ID is required').describe('ID of the message'),
    channel_id: z
      .string()
      .min(1, 'Channel ID is required')
      .describe('ID of the channel the message was sent to'),
    sender_id: z.string().min(1, 'Sender ID is required').describe('ID of the message sender'),
    conversation_id: z
      .string()
      .min(1, 'Conversation ID is required')
      .describe('ID of the conversation this message belongs to'),
    message: z.string().min(1, 'Message content is required').describe('Content of the message'),
    status: z
      .string()
      .min(1, 'Message status is required')
      .describe('Status of the message (e.g., sent, delivered, read)'),
    created_at: DateTimeStringSchema.describe('When the message was created'),
    reply_by: DateTimeStringSchema.optional().describe('When a reply is expected by'),
    conversation_link: z
      .string()
...
export const DeleteConversationSchema = z
  .object({
    channel_id: z.string().min(1, 'Channel ID is required').describe('ID of the channel'),
    conversation_id: z
      .string()
      .min(1, 'Conversation ID is required')
